...
class Database:
    DB_KEYS = ("macid", "is_verified", "stream", "specialty", "discord_name", "discord_id", "is_TA", "is_upper_year")
    DB_TYPES = ("text", "integer", "text", "text", "text", "text", "integer", "integer")
    DB_PROTOTYPE = str(tuple(f"{key} {type}" for key, type in zip(DB_KEYS, DB_TYPES))).replace('\'', '')
    ...
    class User(dict):
        ...
        def __setitem__(self, key, value):
            ...
            cur.execute(f"UPDATE users SET {key} = ? WHERE macid = ?", (value, self["macid"]))
